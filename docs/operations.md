...
`sonar-svc-router-gate` is centered on backend services work in Kotlin. The technical target is: Design a Kotlin verification harness for router systems, covering event replay, fixture event logs, and failure-oriented tests.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 3.
